...
require_once('../../tcpdf/tcpdf.php');
 ...
 $service = new Service;
 $result1= $service->viewOutOfStkBooksOftheSystem();
 $result2= $service->viewOutOfStkProductsOftheSystem();
...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		foreach($data as $row) {
			...
			$this->Cell($w[1], 5, $row[2], 'LR', 0, 'R', $fill);
			$this->Cell($w[2], 5, $row[3], 'LR', 0, 'R', $fill);
			$this->Cell($w[3], 5, $row[4], 'LR', 0, 'R', $fill);
			$this->Ln();
			$fill=!$fill;
		}
...
$html = <<<EOD
<h1>Currently Out of stock Items </h1><br/>
...
//Column titles
$header = array('Item Name', 'Added Date', 'Supplier', 'SupplierTEL');
