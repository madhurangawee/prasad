...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		foreach($data as $row) {
			...
			$this->Cell($w[1], 5, $row[1].' '.$row[2], 'LR', 0, 'L', $fill);
			$this->Cell($w[2], 4, $row[4], 'LR', 0, 'R', $fill);
			$this->Cell($w[3], 5, $row[9], 'LR', 0, 'R', $fill);
			$this->Cell($w[4], 5, $row[11], 'LR', 0, 'R', $fill);
			if($row[14] == 1){
			$this->Cell($w[5], 5, 'Active', 'LR', 0, 'R', $fill);
			}else{
				$this->Cell($w[5], 5, 'Inactive', 'LR', 0, 'R', $fill);
				}
			$this->Ln();
			$fill=!$fill;
		}
...
//Column titles
$header = array('ID','Customer Name', 'City', 'Email', 'Mobile', 'Status');
